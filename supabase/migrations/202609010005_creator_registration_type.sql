...
add column if not exists guild_joined_date date;

alter table public.creator_entities
drop constraint if exists creator_entities_registration_type_check;

alter table public.creator_entities
add constraint creator_entities_registration_type_check
check (registration_type is null or registration_type in ('new_onboarding', 'existing_creator'));

create index if not exists creator_entities_registration_type_guild_joined_date_idx
on public.creator_entities (registration_type, guild_joined_date)
...
create or replace function public.get_visible_creator_registration_metadata(p_creator_entity_ids uuid[])
returns table (creator_entity_id uuid, registration_type text, guild_joined_date date)
language sql
stable
security definer
set search_path = public, pg_temp
as $$
  select entity.id, entity.registration_type, entity.guild_joined_date
  from public.creator_entities entity
  where auth.uid() is not null
    and entity.id = any(coalesce(p_creator_entity_ids, '{}'::uuid[]))
    and exists (
      select 1
      from public.creator_profiles creator
      where creator.creator_entity_id = entity.id
        and (
          (
...
revoke all on function public.get_visible_creator_registration_metadata(uuid[]) from public;
grant execute on function public.get_visible_creator_registration_metadata(uuid[]) to authenticated;
