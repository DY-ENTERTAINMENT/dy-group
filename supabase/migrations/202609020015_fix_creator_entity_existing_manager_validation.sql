...
declare
  ...
  v_current_manager_employee_id uuid;
...
begin
  ...
  select entity.region_id, entity.manager_employee_id
  into v_current_region_id, v_current_manager_employee_id
  ...
  if p_manager_employee_id is distinct from v_current_manager_employee_id
    and not public.is_onboarding_manager_employee(p_manager_employee_id) then
    raise exception 'Invalid manager employee for selected region.';
  end if;
